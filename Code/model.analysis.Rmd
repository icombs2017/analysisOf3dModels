...
#### [GitHub repository](https://github.com/icombs2017/Rhino-Python-3D-Coral-Reefs.git){target="_blank"}


<!--- This is where description of document would go
*** 
# About this document
***
...
For the following analyses we will require the use of a number of different R packages. Most of which can be sourced from CRAN, but some must be downloaded from GitHub. We can use the following code to laod in the packages and install any packages not previously installed in the R console. 

```{r,packages, include = TRUE, message = FALSE, warning = FALSE, results = 'hide}
#setwd("link to github")
if (!require("pacman")) install.packages("pacman")
pacman::p_load(ggplot2, stringr, gridExtra, ggpubr, Rmisc, FSA, rcompanion, RColorBrewer, dplyr, vegan, nparcomp, RVAideMemoire, MANOVA.RM, pairwiseAdonis, PMCMR, PMCMRplus)
...
***
## We found that Site was a significant factor in our PERMANOVA, so we are using a pairwise PERMANOVA to see where differences are between our sites.
...
rate.l1.kw
rate.l2.kw <- compare_means(loss ~ site, data=corr.t3, method = "kruskal.test")
rate.l2.kw
rate.l3.kw <- compare_means(loss ~ site, data=corr.t4, method = "kruskal.test")
rate.l3.kw
```
