...
pacman::p_load(ggplot2, stringr, gridExtra, ggpubr, Rmisc, FSA, rcompanion, RColorBrewer, dplyr, vegan, nparcomp, RVAideMemoire, MANOVA.RM, pairwiseAdonis, PMCMR, PMCMRplus, patchwork, plyr, gdata, flextable, officer)
...
# Subsetting by Sites
***
For the purposes of this manuscript, we want to block certain sites out, early on certain sites were used for exploratory purposes, but were only surveyed once and not included in this study. Here I am only including our thirteen monitoring sites. We are calling it `rd.site` to reflect that. Note: SEFL01 and SEFL02 are also SLR Central and SLR South and were used early on in our exploratory surveys.

```{r, subsetSite, include = TRUE}
...
# Tabulating Data 
***
Currently the data set has very high resolution, to species level. Now, using `dplyr` we will tabulate the data into a more concise form. Sometimes packages mask other packages so I am specifying that I want to use the function `select` from the package `dplyr` by using the following: `dplyr::select`.

```{r, countyData, include = TRUE}
countyData <- rd.site %>% dplyr::select(c(SiteCode, County, MonthYear, SpeciesCode, SCTLD, TotalObservations)) %>% group_by(SiteCode,MonthYear,County) %>% summarise_if(is.integer, sum) %>% ungroup()
countyData 
class(countyData$MonthYear)
countyData$MonthYear <- as.factor(countyData$MonthYear)
class(countyData$MonthYear)
levels(countyData$MonthYear)
countyData$MonthYear <- factor(countyData$MonthYear, levels(countyData$MonthYear)[c(10, 12, 3, 5, 7, 9, 11, 13, 1, 2, 4, 6, 8)])
levels(countyData$MonthYear)
...
set.seed(999)
prev.perm <- adonis(formula = Prevalence ~ County*MonthYear, data = countyData, method = "euclidian", na.rm = TRUE)
```
...
rd.species <- rd.site %>% 
  dplyr::select(c(SpeciesCode, SCTLD, TotalObservations)) %>% 
  group_by(SpeciesCode) %>% summarise_if(is.integer, sum) %>% ungroup()

head(rd.species)

rd.species$Prevalence <- rd.species$SCTLD/rd.species$TotalObservations
head(rd.species)
