...
# We are exploring abundance data, we will do global abundance, and then abundance for each Location
#subsetting to include species data
speciesData <- rd.site %>% dplyr::select(c(SiteCode, Location, County, MonthYear, SpeciesCode, SCTLD, TotalObservations)) %>%  group_by(SiteCode, MonthYear, Location, SpeciesCode) %>% summarise_if(is.integer, sum)
speciesData
...
# just looking at overall specices prevalence
speciesPrev <- speciesData %>% 
              select(c(Location, SpeciesCode, SCTLD, TotalObservations, nonSCTLDObvservations)) %>% 
              group_by(Location,SpeciesCode) %>% 
              summarise_if(is.integer, sum)
...
#adding abundance to the speciesPrev dataframe by diving the total observations for each species by the total coral observations which is found by summing the TotalObservations column
speciesPrev$abundance <- speciesPrev$TotalObservations/(sum(speciesPrev$TotalObservations))*100
speciesPrev <- arrange(speciesPrev, desc(abundance))

globalAbundance <- speciesData %>% 
                    select(c(SpeciesCode, SCTLD, TotalObservations)) %>% 
                    group_by(SpeciesCode)%>% 
                    summarise_if(is.integer, sum)

globalAbundance$Prevalence <- (globalAbundance$SCTLD / globalAbundance$TotalObservations)*100
globalAbundance$abundance <- globalAbundance$TotalObservations / sum(globalAbundance$TotalObservations)*100
globalAbundance <- arrange(globalAbundance, desc(abundance))

kruskal.test(abundance ~ MonthYear, data = globalAbundance)
