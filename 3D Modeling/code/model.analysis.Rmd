...
```{r, permutaionalRM, include = TRUE}
set.seed(999)
aov.total <- aovperm(total.colony.size ~ date*site+Error(colony.id/date), data = corr, np = 9999)
aov.healhty <- aovperm(healthy.area ~ date*site+Error(colony.id/date), data = corr, np = 9999)
aov.disease <- aovperm(disease.area ~ date*site+Error(colony.id/date), data = corr, np = 9999)
aov.lesion <- aovperm(lesion.count ~ date*site+Error(colony.id/date), data = corr, np = 9999)
...
shapiro.test(prop$proportion.of.total)

ggplot(prop, aes(x =t1, y = proportion.of.total))+geom_boxplot()

prop.aov.perm <- aovperm(proportion.of.total ~ t1*site+Error(colony.id/t1), data = prop, np = 9999)

prop.pairwise <- pairwise.wilcox.test(prop$proportion.of.total, prop$t1, p.adjust.method = "bonferroni")
