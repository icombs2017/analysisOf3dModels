...
ggsave("../figures/box.plots.png", plot = box.plots.patch, width = 30, height = 10, units = "in", dpi = 600)
```
...
ggsave("../figures/correlation.plots.png", plot = correlation.plots.patch, width = 20, height = 10, units = "in", dpi = 600)
```
...
```
```{r, lossFriedTab, include = TRUE}
lossFriedTab = data.frame("Data" = "Rate of Tissue Loss", "Test" = "Friedman's Test", "Comparison" = "T1 v T2", "TestStatistic" = rate.l1.kw[["statistic"]][["Kruskal-Wallis chi-squared"]], "p.value" = rate.l1.kw[["p.value"]]) %>% 
...
print(lossKwDoc, target = "../tables/lossKwTab.docx")
lossKwTab
...
ggsave("../figures/loss.plot.png", plot= loss.box, width=15, height=10, units="in", dpi=600)
```
