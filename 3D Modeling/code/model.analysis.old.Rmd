...
ggsave("../figures/S3Fig.png", plot = correlation.plots.patch, width = 10, height = 10, units = "in", dpi = 600)
```
...
shapiro.test(prop$loss)

loss.prop <- friedmanTest(y = prop$loss, groups = prop$t1, blocks = prop$colony.id)
loss.prop

post.prop <- posthoc.friedman.nemenyi.test(loss ~ t1 | colony.id, data = prop) 
prop.total.p.adjust <- p.adjust(post.prop[["p.value"]], method = "bonferroni")

# Making a string of the test statistic and melting 
# it into a column for making tables later
prop.post.total.statistic = c(4.269075, 7.589466,9.012491, 3.320392, 4.743416, 1.423025)
prop.post.total.statistic.melt = melt(prop.post.total.statistic, id = 1)

#taking pvalues from Nemenyi test and manually correcting them
prop.total.val <- c(1.353866e-02,4.801309e-07, 1.113335e-09, 0.087466386, 0.004426398, 0.7457724 )

# Using a bonferroni correction
prop.adjust.total.val <- p.adjust(prop.total.val, method = 'bonferroni')

# Now we are melting this vector (changing it from a row to a column) 
# so we can incorporate into a table later on
prop.p.total.melt <- melt(adjust.total.val, id=1)
...
```{r, propTable, include = TRUE, ft.align = "left"}
propTab = data.frame("Test" = "Proportion of Tissue Lost", "Comparison" = " ", "Statistic" = loss.prop[["statistic"]][["Friedman chi-squared"]], "p.value" = loss.prop$p.value[1]) %>% 
  add_row("Test" = " ", "Comparison" = c("T1 - T2", "T1 - T3", "T1 - T4", "T2 - T3", "T2 - T4", "T3 - T4"), "Statistic" = prop.post.total.statistic.melt$value, "p.value" = prop.p.total.melt$value) %>% 
...
propDoc = read_docx()
propDoc = body_add_flextable(propDoc, value = propTab)
print(propDoc, target = "../tables/TableProp.docx")
propTab
```

```{r, proportion Loss fig, include = TRUE}

# This forces y axis labels to have 1 decimal place.
scale <- function(x) sprintf("%.0f", x)
# Plotting a boxplot of mean total colony size for each site at each timepoint. 
prop.total.box.1 <-
  ggplot(prop.loss, aes(x = t1, y = loss))+
  geom_boxplot(aes(fill = site), alpha = 1, outlier.shape = NA, color = "black") +
  geom_point(aes(fill = site), color = "black", size = 1.75, position = position_jitterdodge()) +
  scale_fill_manual(values = c("#7BA46C", "#EACF9E", "#008D91")) +
  scale_x_discrete(labels = c(expression('T'[2]), expression('T'[3]), expression('T'[4])))+
  labs(x = "Time", y = bquote("Proportion Lost"),fill = 'Site') +
  # scale_y_continuous(labels = scale)+
  theme(legend.title = element_blank(),
        legend.text = element_text(color = "black", size = 18),
        legend.background = element_blank(),
...
prop.total.box <- prop.total.box.1 + theme(
  panel.grid.major = element_line(size = 0.5, linetype = 'solid', colour = "white"),
  panel.background = element_rect(fill = '#F5F5F5'),
  plot.title = element_text(hjust = 0.5), 
  axis.line = element_line(colour = "black"), 
...
ggplot(prop.loss, aes(x = t1, y = loss))+geom_boxplot()
ggplot(prop, aes(x = t1, y = loss))+geom_boxplot()
